...
async function run() {
  ...
  try {
    const migrationFile = join(process.cwd(), "db", "migrations", "016_page_queue_attempts.sql");
    ...
    console.log("\n📦 Running 016 Migration (page_queue.attempts)...");
    ...
    console.log(`✅ Applied ${applied}/${statements.length} statements for 016_page_queue_attempts.sql`);
  } catch (err) {
