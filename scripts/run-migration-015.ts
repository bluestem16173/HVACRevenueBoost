...
async function run() {
  ...
  try {
    const migrationFile = join(process.cwd(), "db", "migrations", "015_page_queue.sql");
    ...
    console.log("\n📦 Running 015 Migration (page_queue)...");
    ...
    console.log(`✅ Applied ${applied}/${statements.length} statements for 015_page_queue.sql`);
  } catch (err) {
