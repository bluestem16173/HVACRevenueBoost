...
export async function generateStaticParams() {
  try {
    ...
    return (diags as any[]).map((d) => ({ symptom: d.slug }));
  } catch {
    return [];
  }
}

export default async function DiagnosticPage({ params }: { params: { symptom: string } }) {
  const res = await sql`
    SELECT id, name, slug, description FROM diagnostics WHERE slug = ${params.symptom} LIMIT 1
  `;
