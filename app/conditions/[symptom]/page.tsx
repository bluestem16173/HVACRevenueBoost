...
export async function generateStaticParams() {
  const staticFromGraph = CONDITIONS.map((c) => ({ symptom: c.slug }));
  ...
  const fromDb = [...dbSymptom, ...dbCondition]
    ...
    .map((p) => ({ symptom: p.slug.replace("conditions/", "") }));
  const seen = new Set(staticFromGraph.map((s) => s.symptom));
  const extra = fromDb.filter((s) => !seen.has(s.symptom));
  return [...staticFromGraph, ...extra];
}

export default async function ConditionPage({ params }: { params: { symptom: string } }) {
  const fullSlug = `conditions/${params.symptom}`;
  let page = await getPageBySlug(fullSlug);
  if (!page) {
    const diagRow = await getDiagnosticPageFromDB(params.symptom, 'condition');
    if (diagRow) {
  ...
  const condition = getCondition(params.symptom);
