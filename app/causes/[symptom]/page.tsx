...
/** /causes/[slug] — Cause pages (AI-generated or from causes table). DB stores full slug: causes/xxx */
export default async function CausesPage({ params }: { params: { symptom: string } }) {
  const causeRes = await sql`
    ...
    WHERE c.slug = ${params.symptom}
    LIMIT 1
  `;

  const aiPage = await getDiagnosticPageFromDB(params.symptom, 'cause');
  ...
  const causeData = causeRes[0] ?? { name: params.symptom.replace(/-/g, " "), explanation: "AI Generated Diagnostic Report", id: null, component_id: null, component_name: null, component_slug: null };
