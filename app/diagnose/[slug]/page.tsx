...
async function getPageWithRetry(slug: string, retries = 2) {
  for (let i = 0; i <= retries; i++) {
    const aiPage = await getDiagnosticPageFromDB(slug, 'diagnose') 
      ?? await getDiagnosticPageFromDB(slug, 'symptom')
      ?? await getDiagnosticPageFromDB(slug, 'condition')
      ?? await getDiagnosticPageFromDB(slug, 'system');
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const aiPage = await getPageWithRetry(params.slug);
...
export default async function SymptomPage({ params }: { params: { slug: string } }) {
  ...
  const fs = require('fs');
  fs.appendFileSync('debug-render.txt', `\n[${new Date().toISOString()}] RENDERING SLUG: ${params.slug} | schema_version: ${aiPage?.schema_version} | page_type: ${aiPage?.page_type}\n`);
