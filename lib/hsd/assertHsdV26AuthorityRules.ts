 ...
 * the renderer depends on (Quick checks scan table, decision columns, cost ladder).
 *
...
export function assertHsdV26AuthorityRules(page: HsdV25Payload): void {
  ...
  if ((page.quick_table ?? []).length < 4) {
    throw new Error("HSD authority: quick_table must have at least 4 rows (Quick checks scan table)");
  }
